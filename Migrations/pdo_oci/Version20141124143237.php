...
class Version20141124143237 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        ...
        $this->addSql("
            ...
            WHERE TABLE_NAME = 'INNOVA_ACTIVITY_QUESTION' 
            AND CONSTRAINT_TYPE = 'P'; IF constraints_Count = 0 
            OR constraints_Count = '' THEN EXECUTE IMMEDIATE 'ALTER TABLE INNOVA_ACTIVITY_QUESTION ADD CONSTRAINT INNOVA_ACTIVITY_QUESTION_AI_PK PRIMARY KEY (ID)'; END IF; END;
        ");
        $this->addSql("
            CREATE SEQUENCE INNOVA_ACTIVITY_QUESTION_ID_SEQ START WITH 1 MINVALUE 1 INCREMENT BY 1
        ");
        $this->addSql("
            CREATE TRIGGER INNOVA_ACTIVITY_QUESTION_AI_PK BEFORE INSERT ON INNOVA_ACTIVITY_QUESTION FOR EACH ROW DECLARE last_Sequence NUMBER; last_InsertID NUMBER; BEGIN 
            SELECT INNOVA_ACTIVITY_QUESTION_ID_SEQ.NEXTVAL INTO : NEW.ID 
            FROM DUAL; IF (
                : NEW.ID IS NULL 
                OR : NEW.ID = 0
            ) THEN 
            ...
            FROM User_Sequences 
            WHERE Sequence_Name = 'INNOVA_ACTIVITY_QUESTION_ID_SEQ'; 
            SELECT : NEW.ID INTO last_InsertID 
            FROM DUAL; WHILE (last_InsertID > last_Sequence) LOOP 
            SELECT INNOVA_ACTIVITY_QUESTION_ID_SEQ.NEXTVAL INTO last_Sequence 
