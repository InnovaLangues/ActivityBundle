    ...
    public function up(Schema $schema)
    {
        ...
        $this->addSql("
            IF EXISTS (
                SELECT * 
                FROM sysobjects 
                WHERE name = 'IDX_DA7D23D581C06096'
            ) 
            ALTER TABLE innova_instruction 
            DROP CONSTRAINT IDX_DA7D23D581C06096 ELSE 
        ...
        $this->addSql("
            IF EXISTS (
                SELECT * 
                FROM sysobjects 
                WHERE name = 'IDX_5C86412B81C06096'
            ) 
            ALTER TABLE innova_question 
            DROP CONSTRAINT IDX_5C86412B81C06096 ELSE 
        ...
        $this->addSql("
            IF EXISTS (
                SELECT * 
                FROM sysobjects 
                WHERE name = 'IDX_660A01D081C06096'
            ) 
            ALTER TABLE innova_proposition 
            DROP CONSTRAINT IDX_660A01D081C06096 ELSE 
        ...
        $this->addSql("
            IF EXISTS (
                SELECT * 
                FROM sysobjects 
                WHERE name = 'IDX_88BEDA0081C06096'
            ) 
            ALTER TABLE innova_information 
            DROP CONSTRAINT IDX_88BEDA0081C06096 ELSE 
            DROP INDEX IDX_88BEDA0081C06096 ON innova_information
        ");
        $this->addSql("
            sp_RENAME 'innova_activityVf.user_id', 
            'author_id', 
            'COLUMN'
        ");
        $this->addSql("
            ALTER TABLE innova_activityVf ALTER COLUMN author_id INT
        ");
        $this->addSql("
        ...
        $this->addSql("
            IF EXISTS (
                SELECT * 
                FROM sysobjects 
                WHERE name = 'IDX_263070EFA76ED395'
            ) 
            ALTER TABLE innova_activityVf 
            DROP CONSTRAINT IDX_263070EFA76ED395 ELSE 
        ...
        $this->addSql("
            IF EXISTS (
                SELECT * 
                FROM sysobjects 
                WHERE name = 'IDX_3635428A81C06096'
            ) 
            ALTER TABLE innova_object 
            DROP CONSTRAINT IDX_3635428A81C06096 ELSE 
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql("
            IF EXISTS (
                SELECT * 
                FROM sysobjects 
                WHERE name = 'IDX_E91F6477F675F31B'
            ) 
            ALTER TABLE innova_activityQru 
            DROP CONSTRAINT IDX_E91F6477F675F31B ELSE 
            DROP INDEX IDX_E91F6477F675F31B ON innova_activityQru
        ");
        $this->addSql("
            sp_RENAME 'innova_activityVf.author_id', 
            'user_id', 
            'COLUMN'
        ");
        $this->addSql("
            ALTER TABLE innova_activityVf ALTER COLUMN user_id INT
        ");
        $this->addSql("
            ALTER TABLE innova_activityVf 
        ...
        $this->addSql("
            IF EXISTS (
                SELECT * 
                FROM sysobjects 
                WHERE name = 'IDX_263070EFF675F31B'
            ) 
            ALTER TABLE innova_activityVf 
            DROP CONSTRAINT IDX_263070EFF675F31B ELSE 
